        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 xl:gap-12">
          {steps.map((step) => (
            <div key={step.id} className="flex flex-col items-center text-center">
              {/* Image */}
              <div className="mb-6">
                <Image src={step.image} alt={step.title} width={255} height={255} className="object-contain" />
              </div>
              ...
              {/* Description */}
              <p className="text-body-base text-foreground max-w-[340px] w-full">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
